	...
	<div class="Hui-article">
		<article class="cl pd-20 container" style="padding-top: 0; width: 75%;">
			...
			<div class="mt-10">
					...
					<tbody>
						...
						<c:forEach items="${page.records }" var="row" varStatus="vs">
							<tr>
								...
								<td><fmt:formatDate value="${row.adopt.adoptDate }" pattern="yyyy-MM-dd HH:mm"/></td>
								<td>${row.pet.subCategory }<span class="c-999">[${row.pet.gender }]</span></td>
								<td>${row.pet.title }</td>
								<td>${row.publisher.nickName }</td>
								<td>${row.adopter.nickName }</td>
								<td>${row.adopt.remark }</td>
									<%-- <c:choose>
				...
				<c:otherwise><a href="adoptmgr?currentpage=1">第1页</a></c:otherwise>
				</c:choose>
				<c:if test="${pageView.currentpage>2}"><a href="adoptmgr?currentpage=${pageView.currentpage-1}">上一页</a></c:if>
				<c:if test="${pageView.currentpage!=1&&pageView.currentpage!=pageView.totalpage}"><b><font>第${pageView.currentpage}页</font></b></c:if>
				<c:if test="${pageView.currentpage<pageView.totalpage}"><a href="adoptmgr?currentpage=${pageView.currentpage+1}">下一页</a></c:if>
				<c:choose>
				<c:when test="${pageView.currentpage<pageView.totalpage}"><a href="adoptmgr?currentpage=${pageView.totalpage}">末页</a></c:when>
